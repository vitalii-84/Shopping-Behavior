...
filters = {
    ...
    "Item Purchased": multi_filter("Придбаний товар", "Item Purchased"),
    ...
    "Subscription Status": multi_filter("Підписка", "Subscription Status"),
    "Shipping Type": multi_filter("Тип доставки", "Shipping Type"),
    "Discount Applied": multi_filter("Знижка застосована", "Discount Applied"),
    "Promo Code Used": multi_filter("Промокод використано", "Promo Code Used"),
    "Payment Method": multi_filter("Спосіб оплати", "Payment Method"),
    "Frequency of Purchases": multi_filter("Частота покупок", "Frequency of Purchases"),
}
...
filtered_df = filtered_df[
    ...
    (filtered_df["Review Rating"] >= rating_range[0]) & (filtered_df["Review Rating"] <= rating_range[1])
]
...
fig_tree.update_traces(insidetextfont=dict(size=16))
...
у більш емоційній формі — через силуети.
""")
...
<div style="display: flex; justify-content: center; align-items: center; gap: 20mm;">
  ...
  <div><img src="https://raw.githubusercontent.com/vitalii-84/Shopping-Behavior/main/woman3.jpg" width="260"/></div>
  <div><img src="https://raw.githubusercontent.com/vitalii-84/Shopping-Behavior/main/man3.jpg" width="260"/></div>
...
# 🔥 Теплова карта взаємозв’язків
st.subheader("📊 Теплова карта взаємозв’язків між змінними")
st.markdown("""
Теплова карта показує силу взаємозв’язків між змінними, включно з категоріальними.
""")
...
sns.heatmap(corr_matrix.astype(float), annot=True, cmap="YlGnBu", linewidths=0.5, fmt=".2f", annot_kws={"size": 8})
...
# 📘 Легенда кольорів для Sankey
st.markdown("""
<style>
.legend-box { display: flex; align-items: center; margin-bottom: 6px; }
.color-square { width: 16px; height: 16px; margin-right: 8px; display: inline-block; border: 1px solid #333; }
</style>
<div class="legend-box"><span class="color-square" style="background-color: rgba(173,216,230,0.6);"></span>
<span>Світло-голубий — <b>Найбільш помітні потоки</b></span></div>
<div class="legend-box"><span class="color-square" style="background-color: rgba(255,255,153,0.6);"></span>
<span>Світло-жовтий — <b>Сезонний зв’язок</b></span></div>
<div class="legend-box"><span class="color-square" style="background-color: rgba(255,182,193,0.6);"></span>
<span>Світло-червоний — <b>Несподівано малий потік</b></span></div>
""", unsafe_allow_html=True)

# ✅ ЄДИНА Sankey Diagram
st.subheader("🔀 Потік покупок: Gender → Category → Season")
st.markdown("""
Ця діаграма показує, як стать покупця впливає на вибір категорії товару, 
а потім — на сезон покупки2.
""")
...
if all(col in filtered_df.columns for col in ["Gender", "Category", "Season"]):
    ...
    fig3 = go.Figure(data=[go.Sankey(
        ...
        link=dict(
            source=source_gc.tolist() + source_cs.tolist(),
            target=target_gc.tolist() + target_cs.tolist(),
            value=value_gc.tolist() + value_cs.tolist(),
            color="rgba(100,100,100,0.3)"
        )
    )])

    fig3.update_layout(title_text="Sankey Diagram: Gender → Category → Season", font=dict(size=14))
